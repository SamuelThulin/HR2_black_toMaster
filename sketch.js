...
function setup() {
  ...
  //need to think strategically about where, what and when to send toMaster(); and about signal flow. For example and lfo is written like this lfo.connect(synth), whereas an effect is like this synth.connect(effect); whatever is in the brackets needs to go toMaster(); you can make it  go toMaster(); elsewhere than in the setup function though
  ...
  amSynth.chain(amVib, amVerb, Tone.Master);
  fmSynth.chain(fmVerb, Tone.Master);
  amSynth2.chain(amVib, autoFilt2.start(), Tone.Master);
  fmSynth2.chain(fmVerb2, autoFilt1, Tone.Master);
  noiseSynth.chain(pingPong1, autoFilt1, Tone.Master);
  noiseSynth.volume.value = -10;
  membraneSynth.chain(fbDelay1, Tone.Master);
  membraneSynth.volume.value = -10;
  noiseOsc.volume.value = -50;
  noiseOsc.chain(Tone.Master);
